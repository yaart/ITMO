...
-- Добавляем поле для времени последнего изменения
ALTER TABLE Radiogram ADD COLUMN last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

-- Создаем функцию триггера
CREATE OR REPLACE FUNCTION check_radiogram_status()
RETURNS TRIGGER AS $$
BEGIN
    -- 1. Проверка активности оборудования
    IF NEW.processing_status_id = 2 AND
       (SELECT status FROM EAL WHERE eal_id = NEW.eal_id) != 'Активен' THEN
        RAISE EXCEPTION 'Нельзя назначить радиограмму на неактивное оборудование';
    END IF;

    -- 2. Проверка доступности сотрудника
    IF NEW.processing_status_id = 2 AND
       (SELECT status FROM Employee WHERE employee_id = NEW.employee_id) != 'Активен' THEN
        RAISE EXCEPTION 'Сотрудник недоступен для обработки радиограммы';
    END IF;

    -- 3. Автоматическое обновление времени изменения
...
-- Создаем триггер
CREATE TRIGGER trg_check_radiogram_status
BEFORE INSERT OR UPDATE ON Radiogram
FOR EACH ROW
EXECUTE FUNCTION check_radiogram_status();


UPDATE Radiogram
SET processing_status_id = 2, eal_id = 3  -- GROK с статусом 'Неактивен'
WHERE radiogram_id = 1;
-- Ошибка: "Нельзя назначить радиограмму на неактивное оборудование"

UPDATE Radiogram
SET processing_status_id = 2, employee_id = 2  -- Кузнецова Ольга в отпуске
WHERE radiogram_id = 1;
-- Ошибка: "Сотрудник недоступен для обработки радиограммы"

UPDATE Radiogram
SET processing_status_id = 2
WHERE radiogram_id = 1;
-- Успешно, поле last_update автоматически обновлено