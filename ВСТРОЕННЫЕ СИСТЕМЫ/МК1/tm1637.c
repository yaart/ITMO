...
void tm1637_display_number(int number) {
  ...
  digits[3] = digit_codes[(number / 1000) % 10];
  digits[2] = digit_codes[(number / 100) % 10];
  digits[1] = digit_codes[(number / 10) % 10];
  digits[0] = digit_codes[ number % 10];

  if (number < 1000) digits[3] = 0x00;
  if (number < 100)  digits[2] = 0x00;
  if (number < 10)   digits[1] = 0x00;

  tm1637_start(); tm1637_write_byte(0x40); tm1637_stop();

  tm1637_start();
  tm1637_write_byte(0xC0);
  tm1637_write_byte(digits[3]);
  tm1637_write_byte(digits[2]);
  tm1637_write_byte(digits[1]);
  tm1637_write_byte(digits[0]);
  tm1637_stop();
